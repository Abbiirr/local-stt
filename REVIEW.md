...
| CR3 | P3 | `controller.py:97-106` | **Resolved by documentation.** `Esc` and overlay close are scoped to the active recording phase; once transcription has started, the overlay can be hidden and transcription continues. This preserves the locked behavior. | None for `v0.1-stable`; cooperative in-flight transcription cancel can be considered later. |
...
No required review fixes remain for `v0.1-stable`. Only user-interactive validation
and optional future hardening remain:
